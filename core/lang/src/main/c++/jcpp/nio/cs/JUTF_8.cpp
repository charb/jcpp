...
namespace jcpp {
	...
	JUTF_8::JUTF_8()
	:	JUnicode(getClazz(), new JString("UTF-8"), null)
	{	}
	...
	JUTF_8::JEncoder::JEncoder(JCharset* cs)
		:	JCharsetEncoder(getClazz(), cs, 1.1f, 3.0f)
	{	}
	...
	JUTF_8::JDecoder::JDecoder(JCharset* cs)
		: JCharsetDecoder(getClazz(), cs, 1.0f, 1.0f)
	{	}
	...
	JCoderResult* JUTF_8::JDecoder::xflow(JBuffer* src, jint sp, jint sl, JBuffer* dst, jint dp, jint nb)
	{
		...
		return  (nb == 0 || sl - sp < nb)?
				const_cast<JCoderResult*>(JCoderResult::JUNDERFLOW) : const_cast<JCoderResult*>(JCoderResult::JOVERFLOW);
	}

	JCoderResult* JUTF_8::JDecoder::xflow(JBuffer* src, jint mark, jint nb)
	{
		JCoderResult* cr = (nb == 0 || src->remaining() < (nb -1))?
				const_cast<JCoderResult*>(JCoderResult::JUNDERFLOW) : const_cast<JCoderResult*>(JCoderResult::JOVERFLOW);
		src->position(mark);
		return cr;
	}
