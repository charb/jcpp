...
namespace jcpp {
	...
	JCharsetEncoder::JCharsetEncoder(jcpp::lang::JClass* _class, JCharset* cs, jfloat averageBytesPerChar, jfloat maxBytesPerChar, JPrimitiveByteArray* replacement)
		: JObject(_class)
	{
	...
	JCharsetEncoder::JCharsetEncoder(jcpp::lang::JClass* _class, JCharset* cs, jfloat averageBytesPerChar, jfloat maxBytesPerChar)
		: JObject(_class)
	{
	...
	void JCharsetEncoder::construct(JCharset* cs, jfloat averageBytesPerChar, jfloat maxBytesPerChar, JPrimitiveByteArray* replacement)
	{
		...
		malformedInputAction = const_cast<JCodingErrorAction*>(JCodingErrorAction::REPORT);
		unmappableCharacterAction = const_cast<JCodingErrorAction*>(JCodingErrorAction::REPORT);
		state = ST_RESET;
	...
	JCharsetEncoder* JCharsetEncoder::onMalformedInput(const JCodingErrorAction* newAction){
		if (newAction==null)
			throw new JIllegalArgumentException(new JString("Null Action"));

		this->malformedInputAction = const_cast<JCodingErrorAction*>(newAction);
		implOnMalformedInput(newAction);
		return this;
	}

	void JCharsetEncoder::implOnMalformedInput(const JCodingErrorAction* newAction){
	}


	JCharsetEncoder* JCharsetEncoder::onUnmappableCharacter(const JCodingErrorAction* newAction){
		if (newAction==null)
			throw new JIllegalArgumentException(new JString("Null action"));

		this->unmappableCharacterAction = const_cast<JCodingErrorAction*>(newAction);
	...
	void JCharsetEncoder::implOnUnmappableCharacter(const JCodingErrorAction* newAction){
	}
	...
	JByteBuffer* JCharsetEncoder::encode(JCharBuffer* in)
	{
		...
		reset();
		for (;;){
			JCoderResult* cr = in->hasRemaining() ?
								encode(in, out, true) : const_cast<JCoderResult*>(JCoderResult::JUNDERFLOW);
			if (cr->isUnderflow())
				cr = flush(out);
